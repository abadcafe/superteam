...
但是superpowers的理念与这冲突很大，他一旦碰上问题就马上跳出来等人类干预，并且他大量使用上下文，长时间跑下来的结果乱七八糟。所以这套东西的核心思想就是 Token 换质量：
...
1. planning阶段会耗费较多时间和较多token，因为实际上在plan阶段很多代码都已经写出来了。如果使用opus 4.6， 万行代码规模的项目耗时在1 - 1.5个小时左右，glm5在这种规模的项目中则很有可能超时失败（executing阶段可继续使用glm5），qwen 3.6 plus能成功但是plan质量不如opus 4.6。我是用的阿里云的coding plan，换个供应商可能结论又不一样。
2. executing阶段我特意放弃了superpowers的并行开发模式，主要是考虑到无人值守模式下耗时长是可以接受的，反正我不跟他交互。正确性对我来说比耗时更重要, 毕竟就算它并行开发也要3个小时以上, 我不打算半夜起来验证它。
3. 暂时放弃了测试覆盖率的要求，后续可以加上 - 实际上这一点还比较重要。
4. 前端等其他场景验证较少。
5. 各种周边系统例如git, ci/cd等, 都没有对接。
