...
试通常有如下原则:
1. 绝不参考任何内部实现, 只通过外部接口测试, 坚决避免调用被测系统内部代码
2. 优先使用真实工具（curl、nc 等）

黑盒测试也被叫做集成测试或者端到端测试, 白盒测试也被叫做单元测试.

## 任务规划(planning)与TDD(Test Driven Development)
...
**注意:**
- 测试用例本身的函数编写过程不用遵循TDD, 但是它用到的辅助函数, 工具等仍然要遵循TDD
- 最后的运行并验证的任务必须也要负责找出测试用例**非预期**失败的根因, 然后修复(修复过程也要TDD)并再次验证
  - 毕竟测试用例或代码实现都有可能出bug
- 所有黑盒测试任务加起来必须完整覆盖所有用户场景(正向/负向场景以及各种边界条件)和所有外部接口

## 黑盒测试规范

1. 所有测试用例必须编写成代码，**禁止手工命令或临时脚本形式的测试用例**
2. 所有测试 mock 必须按测试框架的要求写成代码（例如 pytest fixture）
3. 测试代码必须规范地保存到测试代码目录合适位置
...
2. 编写测试用例时优先选择调用真实工具（如curl、nc等）而不是自己编写类似功能, 因为这样更接近真实场景.
3. 编写测试用例时绝不参考任何功能代码和单测代码, 而只从外部接口的角度来完成编写.
