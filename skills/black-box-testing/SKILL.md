...
6. 确保被测的可执行文件是最新版本且正确运行
7. 规范管理测试产出物，不污染无关目录和环境
8. 测试配置文件应作为测试代码的一部分，不能污染正常配置文件目录
9. 测试用例可独立运行（无用例间依赖）
10. 测试用例可重复运行（无状态残留）
11. 禁止假设测试环境是自己独占, 尽量避免潜在环境冲突.

## 黑盒测试约束

1. 严格遵循项目现有的黑盒测试语言框架以及目录约定, 若无既定惯例则默认使用python + pytest并遵循pytest的目录约定
2. 编写测试用例时优先选择调用真实工具（如curl、nc等）而不是自己编写类似功能, 因为这样更接近真实场景
  - **必须**通过 `--help` 参数等手段充分理解真实工具的所有参数和行为后再编写测试用例, **禁止**基于推测的行为编写测试用例
3. 编写测试用例时**禁止**参考任何功能代码和单测代码, 只能从外部接口的角度来完成编写
