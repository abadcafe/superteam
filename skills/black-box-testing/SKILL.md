...
2. 所有测试用例必须编写成代码，**禁止手工命令或临时脚本形式的测试用例**
3. 所有测试 mock 必须按测试框架的要求写成代码（例如 pytest fixture）
4. 所有测试代码必须规范地保存到测试代码目录合适位置
  - 例如，所有公共 fixture 都在 conftest.py 中定义
5. 编写测试用例时优先选择调用真实工具（如curl、nc等）而不是自己编写类似功能, 因为这样更接近真实场景
  - **必须**先通过 `--help` 参数等手段充分理解真实工具的所有参数和行为后再编写测试用例, **禁止**基于推测的行为编写测试用例
6. 编写测试用例时**禁止**参考任何功能代码和单测代码, 只能从外部接口的角度来完成编写
7. 所有 Python 代码必须有完整的 Type Annotation
