...
- Task 1只能在相关的代码编写、编译、单元测试等任务之前完成, 否则它可能不会失败, 从而违反TDD
...
- 严格遵循项目现有的黑盒测试语言框架以及目录约定, 若无既定惯例则默认使用python + pytest并遵循pytest的目录约定
- 所有测试用例必须写成代码，**禁止手工命令或临时脚本形式的测试用例**
- 所有测试 mock 必须按测试框架的要求写成代码(例如 pytest fixture)
- 所有测试代码必须规范地保存到测试代码目录合适位置
