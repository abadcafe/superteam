...
**在进行各项工作的过程中, 必须充分考虑所有已知问题的已有假设(Assumption). 因此:**
  - 编写或review任务文档之前, 要先读规格文档问题列表.
  - 编写, 运行或review代码和测试用例之前, 要先读任务规划问题列表和环境问题列表.
...
**注意:**
  - 做任务规划时违反规格文档所引起的问题不是规格文档问题禁止记录到规格文档问题列表, 必须马上解决.
...
在**编写代码和测试**的时候, 需要遵循任务文档(task.md)中的任务描述, 如果任务描述中有错漏或
前后矛盾的地方, 那么agent就必须自己做出合理假设来解决并记录问题到任务规划问题列表.

**注意:**
  - 实现者违反任务描述所引起的问题不是任务规划问题禁止记录到任务规划问题列表, 必须马上解决.
  - **做任务规划**和**编写任务文档**时发现的任务规划问题禁止记录到任务规划问题列表, 必须马上解决.
  - 已有代码中的bug就算任务中没有提到也不属于任务规划问题禁止记录到任务规划问题列表, 必须主动解决bug.
