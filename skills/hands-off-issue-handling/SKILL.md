...
后者就需要agent在竭尽全力执行多**种**尝试之后, 再自主决策如何处理. 而agent一旦做出这些决
策, 就必须将这些问题记录下来, 一方面避免不同的agent重复发现已知问题, 另一方面在完成所有工作
之后, 人类仍然能知道发生了什么事情.
...
在编译代码和测试的时候, 可能会因为当前运行环境的问题而失败. agent必须在探索并执行3种以上解
决方案仍然失败之后, 才可以归类为环境问题, 记录下来并继续其他工作.
