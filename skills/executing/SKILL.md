...
- task-reviewer: N calls, N tokens, Nm Ns
...
flowchart TD
  ...
  subgraph task_cycle["Task Cycle"]
    ...
    dispatch_task_reviewer["dispatch task-reviewer"]
    count_pending_issues["
      1. ENSURE task-reviewer dispatched & completed RIGHT BEFORE this step. ONLY THEN:
    ...
    check_test_status -->|"is `EXPECTED`"| dispatch_task_reviewer
    dispatch_task_reviewer --> count_pending_issues
...
- Reorder steps of process flow (Implement → Task review, always)
