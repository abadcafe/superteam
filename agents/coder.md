...
- 阅读集测报告（如有），分析失败场景，解决所有失败用例，即便不在任务目标和架构文档中
- 代码编写过程中如果发现架构问题（不合理/自相矛盾/不清晰），记录到变更总结的 `架构与环境问题` 章节中并继续编写
- 编写符合所有约束的功能代码和单元测试，运行所有单测，并追求单元测试覆盖率100%
- 在任务产出目录中更新变更总结并编写符合规范约束的单测报告，并遵循规范输出结果
